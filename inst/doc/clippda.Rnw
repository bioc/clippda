...
Sample size calculations are typically based on the distribution of the measurement of interest, the between-subject (biological) 
variability, the clinically meaningful size of the difference in expression values between the cases and controls, the power 
required to detect this difference, and the p-value (i.e. false positive rate). In proteomic profiling studies, sample size 
determination 
...
subjects with specific attributes, while the repeated peak information enters as a function of the intraclass correlations 
between the intensities of the replicates. This method makes it straightfoward for a clinician to plan what proportion of samples
 with given attributes to include in an experiment.
...
calculations are: the biological variance, the technical variance, the difference to be estimated, the intraclass correlation between technical replicates, 
significance level, and 
 the power that need to be attained. The power and significance level, and the intraclass correlation (if known) are usually 
pre-specified. The rest of the parameters are computed from the pilot data within the procedure for computing
...
All these parameters are computed within the sample size calculation routine, using the function \texttt{sampleSize}.


\subsection{Sample size calculations}
The sample size can be computed using the function \Robject{sampleSize}. This function, first computes the 
input  parameters for sample size calculation, using the function, \texttt{sampleSizeParameters}. 
The other useful parameter is the heterogeneity correction factor, $Z$, which is the second diagonal element of the 
output matrix 
obtained using the command:

 \texttt{fisherInformation(object)}.

These parameters are then used in the sample size calculations.
...
<<Writing the BibTex.bib file, echo=FALSE>>=
write(paste(
"@Article{Kunetal2006,
...
"@Article{Jorstadetal2007,
    ...
    author = {J\orstad, TS. and Langaas, M. and Bones, AM.},
